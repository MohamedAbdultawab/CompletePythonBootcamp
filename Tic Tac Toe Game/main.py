...
db = {'name1': 1, 'name2': 2, 'name3': 3, 'name4': 4,
      'name5': 5, 'name6': 6, 'name7': 7, 'name8': 8, 'name9': 9}


def print_board(name1, name2, name3, name4, name5, name6, name7, name8, name9):
    os.system('clear')
    print('  {name1} *  {name2}  * {name3}\
        '.format(name1=name1, name2=name2, name3=name3))

    print('*** * *** * ***')

    print('  {name4} *  {name5}  * {name6}\
        '.format(name4=name4, name5=name5, name6=name6))

    print('*** * *** * ***')

    print('  {name7} *  {name8}  * {name9}\
        '.format(name7=name7, name8=name8, name9=name9))
...
def win(db):
    if db['name1'] == db['name2'] == db['name3']:
        m = 'Congratulation X, You Won' if db['name1'] == 'x'\
            else 'Congratulation O, You Won'
        return m
    if db['name4'] == db['name5'] == db['name6']:
        m = 'Congratulation X, You Won' if db['name4'] == 'x'\
            else 'Congratulation O, You Won'
        return m
    if db['name7'] == db['name8'] == db['name9']:
        m = 'Congratulation X, You Won' if db['name7'] == 'x'\
            else 'Congratulation O, You Won'
        return m
    ...
    if db['name2'] == db['name5'] == db['name8']:
        m = 'Congratulation X, You Won' if db['name5'] == 'x'\
            else 'Congratulation O, You Won'
        return m
    if db['name3'] == db['name6'] == db['name9']:
        m = 'Congratulation X, You Won' if db['name3'] == 'x'\
            else 'Congratulation O, You Won'
        return m
...
def replayf():
    ...
    global db
    db = {'name1': 1, 'name2': 2, 'name3': 3, 'name4': 4, 'name5': 5,
          'name6': 6, 'name7': 7, 'name8': 8, 'name9': 9}
    return replay
